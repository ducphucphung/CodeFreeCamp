...
echo "$($PSQL "SELECT year, name FROM teams AS t1 INNER JOIN (SELECT year, winner_id from games WHERE round='Final') AS t2 ON t1.team_id=t2.winner_id ORDER BY year")"
